...
print """
This program is for analyzing data sets for SD-MMEJ consistency.
It generates the same types of information as the program that
analyzes the sequence of the original DSB substrate, but instead of
comprehensively generating and analyzing all possible junctions, it
carries out the analysis on a data set provided by the user.

If you are a new user, please read the instructions for
creating a sequence data file by selecting menu item 1 below.

             ***IMPORTANT***
This program currently handles ONLY blunt or complementary
ended DSB substrates.

SUBSTRATES WITH NONCOMPLEMENTARY OVERHANGING ENDS WILL NOT
BE PROCESSED CORRECTLY.

This program does NOT carry out any validation of user input.
Nonsensical input (such as asking the program to search for
repeats in an interval longer than the DNA sequence provided)
will NOT necessarily cause the program to return an error -
it may return a wrong answer.  Check input carefully!
...
while selection != 3:
    ...
    ***MAIN MENU***
    ...
    1.  Instructions for creating a sequence data file
    2.  Enter data and analysis parameters
    3.  Quit
    """
    selection = input("Type number of desired option and press Return. ")
    if selection == 1:
        print """
        Creating A Sequence Data File:
        
        The file should have one sequence per line.  Do not leave blank lines.
        The end-joining repair junctions should be shown as if they were
        aligned with the original sequence.  Use dashes (-) to indicate
        deleted bases.  example: ATGATG---GCGCTATGCG

        VERY IMPORTANT: For the analysis to work correctly, all of the junction
        sequences in the data file MUST comprise EXACTLY the same sequence
        interval as the sequence that you enter when prompted for the original
        sequence.  In other words,
        
        * All junction sequences MUST be the same length as the original sequence
        
        AND
        
        * There must be an exact one to one correspondence of the remaining
        non-deleted bases in the junction sequences with bases in the original
        sequence.

        example: if you provide the original sequence AAAATTTTGGGGCCCC,
        all of the deletions in your data file must be able to
        align exactly with that sequence (but do NOT include the
        original sequence in the data file):
          AAAATTTTGGGGCCCC (original)
          AAAAT----GGGCCCC (OK)
          AAAATTTTGG-GCCCC (OK)
          CAAAT--TGGGGCCCC (NOT OK - mismatch with the original sequence.)
        CCAAAAT---GGGGCCCC (NOT OK - extra bases in the junction sequence)
            AATTTTG--CCCCC (NOT OK - too few bases in the junction sequence)
        
        ALSO IMPORTANT: To be able to compare the results generated by
        this program with results generated by the program that analyzes
        the original sequence, the original sequence that you provide to this
        program must be the exact same as the original sequence you provide to
        the other program.
        """
    if selection == 2:
        print"""
        ...
        of the original DSB substrate as three subsequences
        corresponding to the sequence to the left of the
        leftmost nick, the sequence between the two nicks,
        and the sequence to the right of the right nick.
        If the sequence is blunt ended, press Return when
        prompted for the sequence between the nicks.
        Do NOT include any spaces or non-base characters.
         """
        #get info from user
        left_dna = raw_input("Input the DNA sequence to the left of the left nick: ")
        left_dna = left_dna.strip()
        left_dna = left_dna.upper()
        mid_dna = raw_input("Input the DNA sequence between the nicks (return if blunt DSB): ")
        mid_dna = mid_dna.strip()
        mid_dna = mid_dna.upper()
        right_dna = raw_input("Input the DNA sequence to the right of the right nick: ")
        right_dna = right_dna.strip()
        right_dna = right_dna.upper()
        search_radius = input("Enter the number of bases to the left and right of the repair junction to search for SD-MMEJ consistent repeats: ")
        min_consist_rep_size = input("Enter the minimum size of an SD-MMEJ consistent repeat: ")

        # do calculations from user input
        leftnick, rightnick = len(left_dna), (len(left_dna) + len(mid_dna))
        original_seq = left_dna + mid_dna + right_dna

        # get and process data file
        data_file = raw_input("Enter data file: ")
        data_file = data_file.strip()
        df = open(data_file)

        # get output filename
        outfile = raw_input("Enter filename for output file: ")
        outfile = outfile.strip()
        outfile = open(outfile, "w")

        outfile2 = raw_input("Enter filename for spreadsheet data: ").strip()
        ...
        for indices in junction_indices_list:
            ejrj = EJR_junction(original_seq, indices, leftnick, rightnick)
        ...
        for ejrj in consistent_ABJ_list:
            ...
            for maxpair in fcr_list:
                ...
                if a_i2 < dl:
                    ...
                    data["side"] = "right"
                    data["rmtobr"] = a_i2 + a_wordlen - leftnick + del_len
                    data["rmtodl"] = a_i2 + a_wordlen - dr + del_len
                    data["p2tobr"] = a_i1 + a_wordlen - leftnick + del_len
                    data["p2todl"] = a_i1 + a_wordlen - dr + del_len
                    data["deltomh"] = dl - leftnick
