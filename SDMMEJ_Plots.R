...
CombinedCurated = subset(reclassData, READS>23) #eliminates all lines with less reads than what we choose to specify 
...
rect_left <- c(33.5,43.5,53.5,63.5,73.5,83.5,93.5,103.5,113.5,123.5,133.5,143.5,153.5,163.5,173.5,183.5,193.5,203.5,213.5)
...
rect_left <- c(43.5,53.5,63.5,73.5,83.5,93.5,103.5,113.5,123.5,133.5,143.5,153.5,163.5,173.5,183.5,193.5,203.5,213.5,223.5)
rectangles <- data.frame(xmin = rect_left,xmax = rect_left + 5,ymin = -Inf,ymax = Inf)
...
InsSide = InsRepeatMotif[,-c(2:45,47:67, 69:75)]
InsSideAgg=aggregate(percent_insertion~SIDE, data=InsSide, sum)
write.csv(InsSideAgg, paste("Table_Outputs/", Plasmid, "_Insertion_Event_Side.csv", sep=""))
...
DeletionResectionPlotSide = ggplot(DelFromBreak)+
  ...
  scale_x_continuous(name="Resection/Unwinding Required for SD-MMEJ Action (bp)",
                     breaks = seq(0, max(DelFromBreak$RM_to_break), by = 5))+
  theme(axis.text.x=element_text(size=10, face="bold"),
        axis.text.y=element_text(size=10, face="bold"),
        axis.title.x=element_text(size=12, face="bold"),
        axis.title.y=element_text(size=12, face="bold"))+
...
AllDeletionsLengthPlot = ggplot(AllDeletionsAggLength)+geom_bar(aes(x=deletion_length, y=Percent_Deletion, fill= CONSISTENCY), stat="identity", colour="black")+
  theme_bw()+scale_y_continuous(name = "Percent of Deletions")+ scale_x_continuous(name = "Deletion Length (bp)", breaks = seq(0, max(AllDeletionsAggLength$deletion_length), by =5))+
  theme(plot.title = element_text(color="grey28", size=12),
        axis.text.x=element_text(size=10, face="bold"),
        axis.text.y=element_text(size=10, face="bold"),
        axis.title.x=element_text(size=12, face="bold"),
...
ConsistentDeletionsLengthPlot = ggplot(ConsistentDeletionsAggLength)+geom_bar(aes(x=deletion_length, y=Percent_Deletion), stat="identity", colour="black", fill="grey50")+
  theme_bw()+scale_y_continuous(name = "Percent of SD-MMEJ Consistent Deletions")+ scale_x_continuous(name = "Deletion Length (bp)", breaks = seq(0, max(ConsistentDeletionsAggLength$deletion_length), by = 5))+
  theme(plot.title = element_text(color="grey28", size=12),
        axis.text.x=element_text(size=10, face="bold"),
        axis.text.y=element_text(size=10, face="bold"),
        axis.title.x=element_text(size=12, face="bold"),
